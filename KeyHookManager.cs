...
public class KeyHookManager : IDisposable
{
    ...
    private long _lastHookActivityMs = -1; // 使用 Environment.TickCount64 替代 DateTime.Now
    ...
    private const long HealthTimeoutMs = 5000; // 5秒超时
...
#pragma warning restore CS0067
    ...
    public bool IsHookResponsive()
    {
        lock (_healthLock)
        {
            if (_lastHookActivityMs < 0)
                return _initialized;
            return (Environment.TickCount64 - _lastHookActivityMs) < HealthTimeoutMs;
        }
    }
