...
public class MainForm : Form
{
    ...
    // _chkOverlap removed: overlap playback feature removed
    ...
    private long _lastActionTicks; // 防止钩子与轮询重复触发
    ...
    private void BuildUI()
    {
        ...
        _trayIcon = new NotifyIcon { Text = "HGmusic", Icon = this.Icon!, ContextMenuStrip = _trayMenu, Visible = false };
        ...
        _pttDetector.PttActivated += () =>
        {
            if (_config.PttEnable && _audio is { IsPlayingAny: false })
            {
                ...
                if (path != null)
                {
                    ...
                    {
                        if (IsDisposed || !IsHandleCreated) return;
                        var bind0 = _bindings.FirstOrDefault(b => b.FilePath == filePath);
                        if (bind0 is { IsPlaying: true }) return;
                        _audio.StopAll();
                        ClearAllPlaying();
    ...
    private void PlayFile(string path)
    {
        ...
        // 如果同一个文件正在播放，则停止它（切换行为）
        var existingBinding = _bindings.FirstOrDefault(b => b.FilePath == path);
        if (existingBinding is { IsPlaying: true })
        {
            _audio.StopAll();
            ClearAllPlaying();
            UpdateFooter();
    ...
    private void HideToTray()
    {
        ...
        {
            _shownTrayTip = true;
            _trayIcon.ShowBalloonTip(3000, "HGmusic", "程序已最小化到系统托盘，仍在后台运行。右键托盘图标可退出。", ToolTipIcon.Info);
        }
    }
    ...
    private void PollKeyState()
    {
        ...
        // 如果钩子刚处理过按键，跳过轮询避免重复触发
        if (DateTime.UtcNow.Ticks - _lastActionTicks < TimeSpan.TicksPerMillisecond * 100) return;
    ...
    protected override void WndProc(ref Message m)
    {
        ...
        // WM_INPUT 不再处理按键：已由低级钩子 (KeyHookManager) 统一处理，避免重复触发
        base.WndProc(ref m);
    }
