...
public static class WaveformRenderer
{
    ...
    public static float[] ExtractPeaks(string filePath, int targetWidth)
    {
        ...
        catch
        {
            ...
            {
                return Enumerable.Repeat(0f, targetWidth).ToArray();
            }
        }
        ...
        var groupSize = (float)frames.Count / targetWidth;
        ...
        for (int i = 0; i < targetWidth; i++)
        {
            ...
            if (start >= end) { peaks[i] = 0f; continue; }

            float max = 0f;
            for (int j = start; j < end; j++)
                max = Math.Max(max, frames[j]);
            peaks[i] = max;
        }
        ...
        while ((read = provider.Read(buffer, 0, buffer.Length)) > 0)
        {
            for (int i = 0; i < read; i += channels)
            {
                ...
                for (int ch = 0; ch < channels && i + ch < read; ch++)
                    maxVal = Math.Max(maxVal, Math.Abs(buffer[i + ch]));
                frames.Add(maxVal);
            }
        }
    }
